...
def lst_doc():
    ...
    l = ["Д. Савченко", "Д. Белова", "Д. Абрамова", "Д. Ефремова", "Д. Олегов", "Д. Димидов", "Д. Октанов", "Д. Леонидов",
...
def ser_avail():
    ...
    l1.place(x=20, y=10)
    f = ["Ультразвук", "Рентген", "Флюрография", "МРТ", "Анализ крови", "Диагностика", "ЭКГ", "Кровь на химию", "Лаборатория"]
    count1 = 20
    for i in f:
        count1 = count1 + 20
...
def modify():
    ...
    if len(a) == 0:
        ...
        l2.place(x=50, y=200)
        l3 = Label(root6, text='1.Имя')
        l3.place(x=50, y=220)
        l4 = Label(root6, text='2.Возраст')
        l4.place(x=50, y=240)
        l5 = Label(root6, text='3.Пол')
        l5.place(x=50, y=260)
        l6 = Label(root6, text='4.Номер тел.')
        l6.place(x=50, y=280)
        l7 = Label(root6, text='5.Группа крови')
        l7.place(x=50, y=300)
...
def do_modify():
    ...
    new = x5.get()
    if choice == '1':
        cur.execute('UPDATE appointment set name={} where name={}'.format(new, ad))
    elif choice == '2':
        cur.execute('update appointment set age={} where age={}'.format(new, ad))
    elif choice == '3':
        cur.execute('update appointment set gender={} where gender={}'.format(new, ad))
    elif choice == '4':
        cur.execute('update appointment set phone={} where phone={}'.format(new, ad))
    elif choice == '5':
        cur.execute('update appointment set bg={} where bg={}'.format(new, ad))
    else:
        pass
    root6.destroy()
...
label = Label(root, text="Регистратура", font="arial 40 bold", bg='#ADD8E6')
b1 = Button(text="Регистрация", font="arial 20 bold", bg='#ADD8E6', command=register)
b2 = Button(text="Запись", font="arial 20 bold", bg='#ADD8E6', command=apoint)
b3 = Button(text="Список врачей", font="arial 20 bold", bg='#ADD8E6', command=lst_doc)
b4 = Button(text="Доступные услуги", font='arial 20 bold', bg='#ADD8E6', command=ser_avail)
b7 = Button(text="Просмотр данных", font='arial 20 bold', bg='#ADD8E6', command=search_data)
b5 = Button(text="Изменить данные", font='arial 20 bold', bg='#ADD8E6', command=mod_sub)
b6 = Button(text="Выход", font='arial 20 bold', command=root.destroy, bg='#ADD8E6')
label.pack()
